    ...
    if ($row = $result->fetch_assoc()) {
        if (!empty($row['password']) && password_verify($password, $row['password'])) {
            ...
            if (empty($row['password'])) {
                $error = "Akun ini terdaftar melalui Google. Silakan login menggunakan tombol Masuk dengan Google.";
            } else {
...
<head>
    ...
    <meta name="description" content="Masuk ke akun NPGLOW Anda untuk belanja skincare dan konsultasi kecantikan.">
...
<body class="bg-slate-50 min-h-screen">
    <div class="flex min-h-screen">
        ...
        <div class="hidden lg:flex lg:w-1/2 image-panel relative items-center justify-center bg-gradient-to-br from-slate-100 to-slate-200">
            <!-- Background Image -->
            <img 
                src="assets/images/image_signin.jpg" 
                alt="NPGLOW Skincare" 
            ...
            <!-- Icon 4: Skincare Droplet -->
            <div class="absolute bottom-[15%] left-[15%] z-10 bg-white/90 backdrop-blur-md p-3 rounded-full shadow-xl text-teal-400 animate-float-wiggle border border-white/50" style="animation-delay: 0.8s;">
            ...
            <div class="absolute bottom-0 left-0 right-0 z-10 p-8 bg-gradient-to-t from-black/40 to-transparent">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-white/80 text-sm">Perawatan Kulit Terbaik di Tangan Ahlinya</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-12 bg-white">
            <div class="w-full max-w-md">
                ...
                <?php if(isset($_GET['success'])): ?>
                    <div class="bg-emerald-50 border border-emerald-200 text-emerald-700 p-4 rounded-xl mb-6 text-sm flex items-center gap-3 fade-in-up">
                ...
                <?php if(isset($_GET['error'])): ?>
                    <?php 
                        $googleErrors = [
                            'google_denied'   => 'Anda membatalkan login dengan Google.',
                            'invalid_state'   => 'Sesi tidak valid. Silakan coba lagi.',
                            'token_failed'    => 'Gagal terhubung ke Google. Silakan coba lagi.',
                            'no_token'        => 'Gagal mendapatkan akses dari Google.',
                            'userinfo_failed' => 'Gagal mengambil informasi akun Google.',
                            'invalid_user'    => 'Data akun Google tidak valid.',
                            'register_failed' => 'Gagal membuat akun. Silakan coba lagi.',
                            'no_code'         => 'Tidak ada kode otorisasi dari Google.',
                        ];
                        $errorMsg = $googleErrors[$_GET['error']] ?? 'Terjadi kesalahan saat login dengan Google.';
                    ?>
                    <div class="bg-red-50 border border-red-200 text-red-600 p-4 rounded-xl mb-6 text-sm flex items-center gap-3 fade-in-up">
                        <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                ...
                <form method="POST" class="space-y-5">
                    ...
                    <div class="fade-in-up fade-in-up-delay-3">
                        ...
                        <div class="relative">
                            ...
                            >
                            <button type="button" onclick="togglePassword('login-password', this)" class="absolute inset-y-0 right-0 pr-3.5 flex items-center password-toggle text-gray-400">
                ...
                <!-- Divider -->
                <div class="my-6 flex items-center gap-4 fade-in-up fade-in-up-delay-4">
                    <div class="flex-1 h-px bg-gray-200"></div>
                    <span class="text-xs text-gray-400 font-medium">Masuk Dengan</span>
                    <div class="flex-1 h-px bg-gray-200"></div>
                </div>

                <!-- Google Button -->
                <a href="auth/google_login.php" class="w-full flex items-center justify-center gap-3 py-3 px-4 border-2 border-gray-200 rounded-xl text-sm font-semibold text-gray-600 hover:border-primary/30 hover:bg-blue-50/50 transition-all fade-in-up fade-in-up-delay-4" id="login-google">
                ...
                <!-- Register Link -->
                <p class="text-center text-sm text-gray-500 mt-8 fade-in-up fade-in-up-delay-5">
                    Belum Punya Akun? 
                    <a href="register.php" class="text-primary font-bold hover:underline hover:text-primary-dark transition-colors">Daftar Disini !</a>
                </p>
            </div>
        </div>
    </div>
